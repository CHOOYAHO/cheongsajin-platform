...
type InterviewStepResponse = { interviewId: string; question: string; feedback?: string; closingSummary?: string; suggestedStrengths?: string[]; decision?: InterviewDecision; score?: number; aiSource?: 'openai' | 'fallback'; status: 'inProgress' | 'completed' }
...
function AiInterviewActivity({ schoolName, displayName }: { schoolName: string; displayName: string }) {
  ...
  return (
    <section className="ai-interview-panel">
      ...
      {phase === 'interview' && selectedCompany && <div className="ai-interview-room"><div className="interview-status"><span>{selectedCompany.name}</span><b>{application.role === '직접 입력' ? customRole : application.role} 면접</b><small>{turns.length > 0 ? `${turns.length}번 답변함` : '진행 중'}</small></div>{lastFeedback && <div className="interview-feedback"><b>방금 답변 피드백</b><p>{lastFeedback}</p></div>}<div className="interview-chat-log">{turns.map((turn, index) => <article key={`${turn.question}-${index}`}><div className="chat-bubble interviewer"><span>면접관</span><p>{turn.question}</p></div><div className="chat-bubble applicant"><span>나</span><p>{turn.answer}</p></div></article>)}</div><div className="interview-question"><span>AI 면접관</span><h3>{currentQuestion}</h3></div><label>내 답변<textarea value={answer} onChange={(event) => setAnswer(event.target.value)} maxLength={1200} placeholder="지원자처럼 답변해 보세요." /></label>{error && <p className="entry-error" role="alert">{error}</p>}<div className="interview-actions"><button type="button" className="secondary" onClick={() => void submitAnswer(true)} disabled={isBusy}>{isBusy ? '결과 정리 중' : '면접 마치고 결과 보기'}</button><button type="button" onClick={() => void submitAnswer(false)} disabled={isBusy}>{isBusy ? '다음 질문 만드는 중' : '답변 보내기'}</button></div></div>}
    ...
    <div className="app-shell">
      ...
      <main className="dashboard">
        <section className={`dashboard-intro ${isMentorView ? 'has-question-action' : ''}`}>
          ...
          {isMentorView && <button type="button" className="mentor-inbox-button" onClick={() => setShowQuestionInbox(true)}><span>💬</span><div><small>학생 질문</small><b>질문 확인하기</b></div>{mentorQuestions.filter((item) => item.status === 'waiting').length > 0 && <em>{mentorQuestions.filter((item) => item.status === 'waiting').length}</em>}</button>}
        ...
        {showQuestionInbox && isMentorView && <MentorQuestionInbox questions={mentorQuestions} onClose={() => setShowQuestionInbox(false)} onRead={(question) => void markMentorQuestionRead(question)} />}
