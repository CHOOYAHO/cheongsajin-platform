...
type AuctionResultRecord = { id: string; userId?: string; roomCode: string; displayName: string; selectedJob: string; balance: number; inventory: Record<string, number>; selectedJobs?: string[]; deck?: string[]; auctionIndex: number; totalItems: number; endedByHost?: boolean; savedAt?: { toMillis: () => number } }
...
function AdminAuctionResultsPanel() {
  ...
  useEffect(() => {
    ...
    getDocs(collection(db, 'auctionResults')).then((snapshot) => {
    ...
    }).catch((loadError) => { console.error(loadError); setError('전체 경매 결과를 불러오지 못했어요.') }).finally(() => setLoading(false))
  }, [])
  if (loading) return <div className="admin-placeholder-note"><b>경매 결과 확인 중</b><p>Firebase 데이터베이스에 저장된 전체 기록을 불러오고 있어요.</p></div>
  if (error) return <p className="entry-error" role="alert">{error}</p>
  return <section className="admin-auction-results"><div className="admin-auction-summary"><div><small>저장된 결과</small><b>{records.length}건</b></div><div><small>경매방</small><b>{new Set(records.map((record) => record.roomCode)).size}개</b></div><div><small>참가자</small><b>{new Set(records.map((record) => record.userId || record.displayName)).size}명</b></div></div>{records.length ? <div className="admin-auction-table">{records.map((record) => <article key={record.id}><header><div><span>방 {record.roomCode}</span><b>{record.displayName}</b></div><small>{record.savedAt?.toMillis ? new Date(record.savedAt.toMillis()).toLocaleString('ko-KR') : '저장 시간 미기록'}</small></header><dl><dt>선택 직업</dt><dd>{record.selectedJob || '미기록'}</dd><dt>진행 상품</dt><dd>{record.auctionIndex} / {record.totalItems}</dd><dt>남은 포인트</dt><dd>{record.balance}P</dd><dt>종료 방식</dt><dd>{record.endedByHost ? '방장 종료' : '정상 종료'}</dd></dl><div className="admin-strength-list">{Object.entries(record.inventory ?? {}).length ? Object.entries(record.inventory ?? {}).map(([strength, count]) => <span key={strength}>{strength} <b>{count >= 3 ? 'EPIC' : count === 2 ? 'RARE' : 'NORMAL'}</b></span>) : <span>낙찰받은 강점 없음</span>}</div></article>)}</div> : <div className="empty-auction-records"><b>Firebase에 저장된 강점 경매 결과가 없어요.</b><p>게임이 결과 단계까지 완료되면 이곳에 저장됩니다.</p></div>}</section>
}
