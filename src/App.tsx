...
function App() {
  ...
  const completedSessionCount = school === 'yesan-high' || isStaffAccount ? 1 : 0
  ...
  if (activeSession === 2 && sessionPageMode === 'activity' && activeSecondActivity) {
    ...
    const viewerMode = isAdminMode ? 'all' : isTeacherMode || normalizedName === '예산고' || normalizedName === '광시중' ? 'school' : 'student'
    ...
    <div className="app-shell">
      ...
      <main className="dashboard">
        ...
        <section className="dashboard-guide" aria-label="청사진 안내 메뉴">
          ...
          <button type="button" onClick={() => openGuide('profile')}><span className="guide-icon green">👤</span><div><small>{isMentorMode || isAdminMode ? '멘토 정보' : '나의 정보'}</small><h2>{isMentorMode || isAdminMode ? '멘토 프로필 작성' : '프로필 작성'}</h2><p>{isMentorMode || isAdminMode ? '멘토 소개 화면에 표시할 내 정보를 작성해요.' : '나를 소개하고 관심 분야와 희망 진로를 기록해요.'}</p></div><b>→</b></button>
